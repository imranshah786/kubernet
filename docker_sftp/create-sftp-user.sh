...
if [ -n "$dir" ]; then
    ...
    for dirPath in "${dirArgs[@]}"; do
        ...
        if [ ! -e "$dirPath" ]; then
            log "Creating symbolic link: $dirPath -> /data"
            ln -s /data "$dirPath"
            if [ $? -eq 0 ]; then
                log "Symbolic link created successfully: $dirPath"
                # Cambia el propietario y los permisos del enlace simbólico
                chown -h "$user" "$dirPath"
                chmod 755 "$dirPath"
                if [ $? -eq 0 ]; then
                    log "Permissions updated for symbolic link: $dirPath"
                else
                    log "Failed to update permissions for symbolic link: $dirPath"
                    exit 1
                fi
            else
                log "Failed to create symbolic link: $dirPath"
                exit 1
            fi
        else
            log "Symbolic link already exists: $dirPath"
        fi
    done
fi
